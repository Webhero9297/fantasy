...
class EthLibrary
{
    ...
    private function initWeb3() {
        $provider_type = env('ETH_PROVIDER');
        $contractAddress = '';
        if ( strtolower($provider_type) == 'test' ) {
            $eth_provider = env('ETH_TESTPROVIDER');
            $this->contractAddress = env('ETH_TESTCONTRACTADDRESS');
        }
        else{
            $eth_provider = env('ETH_MAINPROVIDER');
            $this->contractAddress = env('ETH_LIVECONTRACTADDRESS');
        }
        $this->web3 = new Web3(new HttpProvider(new HttpRequestManager($eth_provider, 2)));
